...
function CreateNewTodo({selectedProject, refreshTodosList, setRefreshTodosList}: CreateNewTodoProps): JSX.Element {
    ...
    return(
        <div id="create-new-todo">
            {mode === "button" && <>
                <button type="button" onClick={() => setMode("input")}>Create New Todo</button>
            </>}
            {mode === "input" && <form>
                <label htmlFor="title">Title</label>
                <input type="text" id="title" value={newTodo.title} onChange={(e) => {setNewTodo({...newTodo, title: e.target.value})}}/>
